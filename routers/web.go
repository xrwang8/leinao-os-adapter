...
import (
	...
	"leinao-os-adapter/pkg/http/controller/captcha"
	"leinao-os-adapter/pkg/http/middleware/authorization"
	"leinao-os-adapter/pkg/http/middleware/cors"
	validatorFactory "leinao-os-adapter/pkg/http/validator/core/factory"
	"leinao-os-adapter/pkg/utils/gin_release"
	"net/http"
...
func InitWebRouter() *gin.Engine {
	...
	backend := router.Group("/admin/")
	{
		...
		{
			// 关于路由的第二个参数用法说明
			// 1.编写一个表单参数验证器结构体，参见代码：   pkg/http/validator/web/users/register.go
			// 2.将以上表单参数验证器注册，遵守 键 =》值 格式注册即可 ，pkg/http/validator/common/register_validator/web_register_validator.go  20行就是注册时候的键 consts.ValidatorPrefix+"UsersRegister"
